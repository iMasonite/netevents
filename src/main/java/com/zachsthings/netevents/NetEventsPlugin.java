...
public class NetEventsPlugin extends JavaPlugin {
  /** Number of event UUID's to keep to prevent duplicate events. Greater number potentially
   * decreases duplicate events received. */
  ...
  /** Returns a persistent unique ID for this server. Useful for identifying this server in the
   * network. Persisted to {@code {@link #getDataFolder()}/uuid.dat}.
   * 
  ...
  /** Calls the passed event on this server and forwards it to all connected servers to be called
   * remotely. Events must be serializable. Remote servers will ignore events they do not know the
   * class of.
   * 
  ...
  public synchronized void callEvent(EventPacket packet, Forwarder ignoreTo) {
    ...
    if (processedEvents.contains(packet.getUid())) return;
