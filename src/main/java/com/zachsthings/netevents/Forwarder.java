...
/** Handles logic of connection management and teardown. Wraps connection a lot, but handles the
 * logic of state tracking in {@link com.zachsthings.netevents.NetEventsPlugin} and autoreconnect. */
public class Forwarder implements Closeable {
  ...
  public void setRemoteServerUUID(UUID remoteUid) {
    if (!remoteServerUUID.compareAndSet(null, remoteUid)) throw new IllegalStateException("Server UUID has already been set for " + this);
  }
  ...
  public SocketAddress getRemoteAddress() {
    final Connection conn = this.conn.get();
    if (conn != null) return conn.getRemoteAddress();
    else return reconnectAddress;
  }
  
  @Override
  public String toString() {
    return "Forwarder{" + "conn=" + conn + ", reconnectAddress=" + reconnectAddress + '}';
  }
}
