...
/** Represents a single connection. {@link Forwarder} wraps connection for reconnecting, but once
 * this is closed it's closed permanently. */
class Connection implements Closeable {
  ...
  private Connection(Forwarder attachment, SocketChannel chan) throws IOException {
    ...
    if (remoteAddress == null) throw new IOException("Null remote address for " + chan);
  }
  ...
  public void write(Packet p) {
    if (!chan.isConnected()) throw new IllegalStateException("Channel not connected");
    out.sendQueue.addLast(new PacketEntry(p, false));
  }
  
  public void writeAndClose(Packet p) {
    if (!chan.isConnected()) // We're assuming that the channel has been disconnected from the other
                             // side,
  ...
  public String toString() {
    return "Connection{" + "closeListeners=" + closeListeners + ", chan=" + chan + ", disconnectHandled=" + disconnectHandled + '}';
  }
  ...
  public class InputThread extends IOThread {
    ...
    public void act() throws IOException {
      ...
      while (read < len) {
        ...
        if (tempRead == -1) throw new ClosedChannelException();
        read += tempRead;
      }
      payload.flip();
